...
{
    public class GenericSqliteRepository<T> : IGenericRepository<T> where T : BaseModel, new()
    {
        ...
        public virtual async Task<int> ExecuteScriptsScalarAsync(string query, object[] args)
        {
            return await _context.Database.ExecuteScalarAsync<int>(query,args);
        }
        public virtual async Task<List<T>> ExecuteScriptsWithParameterAsync(string query,object[] args)
        {
            return await _context.Database.QueryAsync<T>(query,args);
        }
    }
}
